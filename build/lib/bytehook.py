...
def remove_hookpoint(num):
   ...
   origin[num][0].__code__ = origin[num][1]
   del origin[num]
   if num in hookpoints:
       del hookpoints[num]
...
def hook_modifyRetval(func, retVal=None):
    ...
    origin[hookpointcounter] = (func, func.__code__)
    hookpointcounter += 1
    ...
    newfunc = types.CodeType(code.co_argcount, code.co_kwonlyargcount, code.co_nlocals, code.co_stacksize, code.co_flags, newcode, newconsts, code.co_names, code.co_varnames, code.co_filename, code.co_name, code.co_firstlineno, newlnotab, code.co_freevars, code.co_cellvars)
